...
actor CatalogWorker {
    ...
    /// cada álbum probable, otro para las que no encajan en ninguno y otro para los duplicados
    /// de fotos ya clasificadas.
    /// - Parameters:
    ///   - group: nombre del grupo ("Sin clasificar").
    ///   - fallbackAlbum: subálbum para las fotos sin álbum probable.
    ///   - duplicatesAlbum: subálbum para las copias repetidas de fotos que ya están en un álbum.
    /// - Returns: variantes añadidas de verdad (contadas en Capture One), las que no entraron y subálbumes usados.
    func createUnfiledAlbums(group: String, fallbackAlbum: String, duplicatesAlbum: String, images: [UnfiledImage]) throws -> (added: Int, failed: Int, albums: Int) {
        ...
        let byAlbum = Dictionary(grouping: images) { $0.duplicateInAlbum != nil ? duplicatesAlbum : ($0.suggestion?.album ?? fallbackAlbum) }
        var added = 0, expected = 0
        for (album, photos) in byAlbum.sorted(by: { $0.key < $1.key }) {
            let path = [group, album]
            // No se filtra por nombre contra lo que el subálbum ya tiene: varias copias importadas
